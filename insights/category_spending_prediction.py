...
async def get_average_monthly_purchases_three_months(user: User, db: Session):
...
async def calculate_spending_probability(user, db, category, threshold, T=3, shape_param=2):
    ...
    if category == PurchaseCategory.GENERAL:
        ...
        total_future_spend = gamma.rvs(shape_param, scale=scale_param, size=1000) * T
        ...
        def projected_category_spend(category, total_future_spend):
            p_category = category_proportions.get(category, 0)
            category_spend = total_future_spend * p_category  # Estimated spend for category
            return category_spend

        def probability_exceeding_threshold(category, threshold, total_future_spend):
            category_spend = projected_category_spend(category, total_future_spend)
            prob_exceed = np.mean(category_spend > threshold)  # P(X_C > V)
            return prob_exceed

        prob_exceed = probability_exceeding_threshold(category, threshold, total_future_spend)
